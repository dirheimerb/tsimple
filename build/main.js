 ...
 * @return {void} - Does not return a value.
 */
const executeShell = (scriptContent) => {
    try {
...
program
    ...
    .addOption(new Option('-w, --workspace', 'Install dependencies in a workspace').default(false))
    ...
    .addHelpText('afterAll', chalk.cyanBright("Run -d first or use 'pnpm install -D @babel/core @babel/cli @babel/preset-typescript @babel/preset-env typescript ts-node @types/node tsx\r\n After dependencies are installed run -r to generate .babelrc file and finally run -t to generate tsconfig.json file"));
